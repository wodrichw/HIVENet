...
def your_photos():
    ...
    cmd = """ python ../facenet/align_data.py > /dev/null 2>&1 
    rm static/current_face/* > /dev/null 2>&1 
    cp ../facenet/aligned_data/"+it.name+"/* static/current_face/ 
    ls static/current_face
    """
...
def retake_photos():
    ...
    subprocess.Popen("rm -r ../facenet/aligned_data/"+it.name+" ../data/"+it.name+"/*", shell=True)
    return render_template('take_face_photos.html')
...
def submit_photos():
    ...
    subprocess.Popen('python ../facenet/classify.py ; cp '+nodedir+'/* ../communication/client/assets & python ../communication/client/client.py', shell=True)
