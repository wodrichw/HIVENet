...
def matchName(model,HumanNames,face):
    ...
    result_names =''
    best_class_probabilities[0] = best_class_probabilities[0] + (.0387 * len(HumanNames))
...
def recognize(RD=os.path.dirname(os.path.realpath(__file__))):
            ...
            while True:
                ...
                if nrof_faces > 0:
                    ...
                    for i in range(nrof_faces):
                        ...
                        text_x = bb[i][0]
                        text_y = bb[i][3] + 20
                        texttoOutput = name + " " + str(math.trunc(sum(results[name]) / len(results[name]))) 
