...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        num_correct =0
        total_analyzed = 0
        for sub_dir, dir, files in os.walk(testing_directory):
            for file in files:
                file_to_analyze = os.path.join(sub_dir,file)
                print (file_to_analyze)
                first_name_test = file.split('_')[0]
                #ret, frame = video_capture.read()
                frame = cv2.imread(file_to_analyze)
                ...
                if nrof_faces > 0:
                    ...
                    for i in range(nrof_faces):
                        ...
                        print ("name1:" + first_name_test + ", name2: "+ max_name_result[0])
                        if max_name_result[0] == first_name_test:
                            num_correct += 1 
                            total_analyzed +=1
                            ...
                            total += 1
                            print("Identified incorrectly. Expected: " + first_name_test + ", identified as: " + max_name_result[0])
                else:
                    print('Unable to align')
        print("inference engine scored "+ str(((num_correct/total_analyzed)*100)) + " percent accuracy.")
        print("inference engince got "+ str(num_correct) +" out of " + str(total_analyzed) + " correct.")
